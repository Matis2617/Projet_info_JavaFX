...
public class OperateurFormView {
    ...
    public static Node getOperateurForm(
        ...
        associerBtn.setOnAction(e -> {
            ...
            boolean posteOccupe = poste.getOperateur() != null;
            if (machineOccupee || posteOccupe) {
                infoLabel.setText("Impossible : le poste ou une machine est occupé !");
                return;
            }
        ...
        rightBox.getChildren().addAll(
                listeTitre, tableOp,
                assocTitre, new HBox(10, comboPostes, associerBtn, infoLabel),
                new HBox(10, supprimerBtn, modifierBtn)
        );
...
Module M2
PROJET
Gestion de la fabrication d’une ligne de produits
Etape 3
Analyse de la fiabilité des machines
Le gestionnaire d’un atelier de fabrication souhaite analyser la fiabilité des machines à partir
d’un carnet de maintenance enregistré dans un fichier texte, suiviMaintenance.txt dont la
structure est indiquée ci-dessous.
Date Heure Machines Evénements Opérateur Cause
20012020 06 :42 Mach_5 A OP102 panne
20012020 07 :34 Mach_3 A OP110 panne
20012020 11 :40 Mach_5 D OP213 x
20012020 12 :18 Mach_2 A OP103 accident
20012020 13 :25 Mach_4 A OP221 maintenance
20012020 15 :12 Mach_3 D OP123 x
20012020 16 :27 Mach_4 D OP114 x
15032020 16 :36 Mach_5 A OP320 panne
15032020 18 :20 Mach_5 D OP210 x
02042020 18 :35 Mach_1 A OP321 panne
02042020 19 :32 Mach_1 D OP102 x
-
-
-
End Of File
Chaque événement est enregistré en identifiant la machine et en indiquant l’heure, l’opérateur
et la cause.
Ces différentes informations sont stockées dans le fichier en les séparant par un espace.
Fonctionnement de l’atelier
L’atelier comporte n machines à préciser
Heure de démarrage : 06H00
Heure d’arrêt : 20H00
Département SeC
Fiabilité Machine Marche/Arrêt
Début FinA D A D
Période d’observation
Travail demandé:
1°) Ecrire un algorithme général pour calculer la fiabilité pour chacune des n machines.
2°) Ecrire un algorithme pour trier les machines par ordre décroissant de fiabilité.
3°) Ecrire le programme JAVA correspondant.
Proposition de solution
1°) Identification des machines à analyser
Soit M[n] un tableau des n machines
e=0 : entier (nombre de machines enregistrées dans le tableau M[]
ajouter=oui booléen pour enregistrer une nouvelle référence de machine (sinon ajouter=non).
Tantque (non fin du fichier suivi.txt)
{ Lire le fichier de suivi.txt
Décomposer la lignelue
Lignelue=in.readLine() ;
StringTokenizer t=new StringTokenizer (lignelue, “ ”)
machine= t.nextToken()
//Vérification de l’existence de la machine lue dans le tableau de machines déjà enregistrées
Si (e=0)
Alors
{
 M[0]=machine
...
Pour (i=0 ; i<e ; i=i+1)
{
Si (machine.equals(M[i])==true) Alors ajouter=non ;
}
}
Si (ajouter=1) Alors
{e=e+1
...
2°) Extraction des événements associés à chaque machine
3°) Calcul de la fiabilité de chaque machine
4°) Tri des machines par ordre décroissant de fiabilité 
